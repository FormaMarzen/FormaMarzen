...
export default function RodzajeZajecPage() {
  ...
  const loadRodzaje = async () => {
    try {
      const { data, error } = await supabase
        .from('rodzaje_zajec')
        .select('*')
        .order('id', { ascending: true }); // Zmiana na 'id', żeby sortowało domyślnie wg kolejności dodania (jeśli brak created_at)
      ...
      if (data) {
        const parsedData = data.map((item: any) => {
          ...
          return {
            ...
            // W zależności od definicji tabeli w Supabase, możesz nie mieć kolumny created_at. Jeśli nie, podstaw dzisiejszą datę dla celów wizualnych:
  ...
  // 2. ZAPIS DO SUPABASE (NOWY LUB AKTUALIZACJA)
  const handleAddOrUpdateRodzaj = async (e: React.FormEvent) => {
    ...
    const payload: any = {
      nazwa: nazwa,
      kolor: kolor,
      limit_miejsc: 12, // Defaultowy limit 12 miejsc, można dorobić na to input w przyszłości, zależy od potrzeb
    };

    // Zmodyfikuj tabelę, aby dodać kolumnę ustawienia. Jeśli już utworzyłeś tabelę "rodzaje_zajec" poleceniem sql z poprzedniej rozmowy
    // Zrób to przez wykonanie szybkiego zapytania w SQL Editor na stronie Supabase:
    // ALTER TABLE rodzaje_zajec ADD COLUMN ustawienia JSONB;
    // Jeśli już masz to pole, to się po prostu zapisze
    payload['ustawienia'] = metaDane;
    ...
    try {
      if (editingId !== null) {
        ...
        const { error } = await supabase
          .from('rodzaje_zajec')
          .insert([payload]); // Supabase wygeneruje ID jeśli jest BIGINT GENERATED BY DEFAULT
      ...
      alert(`Wystąpił błąd podczas zapisu: ${error.message || JSON.stringify(error)}. \nUpewnij się, że dodałeś kolumnę 'ustawienia' typu JSONB w panelu Supabase (ALTER TABLE rodzaje_zajec ADD COLUMN ustawienia JSONB;).`);
    }
  };
  ...
  // Komponent pomocniczy usuwający CZYTAJ WIECEJ i dostosowujący wygląd do screena
