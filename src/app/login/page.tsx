...
import { supabase } from '../raporty/klienci/supabase'; // Upewnij się, że ścieżka do supabase.ts jest poprawna
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Autoryzacja przez wbudowany system Supabase
    ...
    if (error) {
      ...
      // Sukces! Przekierowanie do panelu głównego
      router.push('/'); 
    }
  };

  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-between p-4 sm:p-8 font-sans antialiased text-slate-800 relative overflow-hidden">
      
      {/* Tło dekoracyjne po prawej stronie (linie faliste jak na zrzucie) */}
      ...
      <div className="w-full max-w-md flex justify-end z-10">
        <button className="flex items-center gap-1.5 text-xs font-semibold text-slate-600 hover:text-slate-900 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm cursor-pointer transition-colors">
          <span>🌐</span> Polski <span>▼</span>
        </button>
      </div>
      ...
      <div className="w-full max-w-md space-y-6 my-auto z-10">
        ...
        <div className="flex flex-col items-center space-y-3">
          <div className="w-20 h-20 bg-white rounded-2xl border border-slate-200 shadow-sm flex items-center justify-center p-3">
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
          ...
          {/* Wyświetlanie błędu autoryzacji */}
          {errorMsg && (
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm space-y-2 text-xs text-slate-600">
          ...
          <ul className="space-y-1 pl-1">
            <li className="flex items-center gap-2 text-sky-600 hover:underline cursor-pointer">
              <span className="w-1.5 h-1.5 bg-sky-600 rounded-full"></span> Zapisz się na pierwsze zajęcia bez kupna karnetu
            </li>
            <li className="flex items-center gap-2 text-sky-600 hover:underline cursor-pointer">
              <span className="w-1.5 h-1.5 bg-sky-600 rounded-full"></span> Zarejestruj konto klubowicza i kup karnet
            </li>
          </ul>
        </div>
      ...
      {/* Stopka */}
      <div className="w-full text-center text-[11px] text-slate-400 py-2 z-10">
        Dumnie wspierany przez WodGuru 🖤
      </div>
