...
export default function RegulaminPage() {
  ...
  const handleOpenModal = (regulation: Regulation, edit: boolean = false) => {
  ...
  const handleSaveRegulation = async () => {
    ...
    if (!error) {
      setRegulations(regulations.map(r => r.id === selectedRegulation.id ? { ...r, content: editContent } : r));
      handleCloseModal();
    } else {
      console.error(error);
          ...
          <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-2xl">
              <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                {isEditMode ? 'Edycja dokumentu:' : 'Podgląd dokumentu:'} {selectedRegulation.title}
              </h3>
              <button 
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {isEditMode ? (
                <div className="h-full flex flex-col gap-2">
                  <label className="text-sm font-semibold text-slate-600">Treść regulaminu</label>
                  <textarea
                    className="w-full flex-1 min-h-[300px] p-4 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none resize-none text-slate-700"
                    value={editContent}
            ...
            <div className="p-6 border-t border-slate-100 flex justify-end gap-3 bg-slate-50 rounded-b-2xl">
              ...
              {isEditMode ? (
                ...
                !isAdmin && !isAccepted(selectedRegulation.slug) && (
                  <button 
