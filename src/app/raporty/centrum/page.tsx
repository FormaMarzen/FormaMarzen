...
interface KarnetGroup {
  nazwa: string;
  ilosc: number;
  lacznaWartosc: number;
}
...
export default function ReportsCenterPage() {
  ...
  const [stats, setStats] = useState({
    totalKlubowicze: 0,
    zWaznymKarnetem: 0,
    karnetyZestawienie: [] as KarnetGroup[],
    aktywneUmowy: 0,
    przychodStalyUmowy: 0,
    umowyZestawienie: [] as KarnetGroup[],
    chartData: [] as ChartData[]
  });

  // Funkcja pomocnicza weryfikująca, czy dany karnet to umowa długoterminowa
  const isKarnetNaUmowe = (karnet: any): boolean => {
    if (!karnet) return false;
    if (karnet.isContract === true || karnet.czyUmowa === true || karnet.typ === 'umowa' || karnet.rodzaj === 'umowa') {
      return true;
    }
    const nazwa = String(karnet.nazwa || '').toLowerCase();
    return nazwa.includes('umow') || nazwa.includes('12 miesi') || nazwa.includes('6 miesi') || nazwa.includes('cyklicz');
  };
  ...
  useEffect(() => {
    const fetchReportData = async () => {
      ...
      try {
        ...
        let aktywneUmowy = 0;
        let przychodStalyUmowy = 0;

        const karnetyMap: Record<string, { ilosc: number, sumaCen: number }> = {};
        const umowyMap: Record<string, { ilosc: number, sumaCen: number }> = {};
        ...
        if (klienciData) {
          ...
          klienciData.forEach(klient => {
            ...
            parsedKarnety.forEach((karnet: any) => {
              // Weryfikacja czy karnet jest aktywny na dzień dzisiejszy
              const isAktywny = karnet.waznyDo && karnet.waznyDo >= todayStr && !karnet.zawieszonyOd && !karnet.statusTekst?.includes('Oczekujący');

              if (isAktywny) {
                maAktywnyKarnet = true;
                
                // Odczytanie indywidualnej ceny karnetu przypisanej u danego klubowicza
                const cenaStr = karnet.cenaMiesieczna || karnet.rata || karnet.cena || '0';
                const cenaNum = parseFloat(String(cenaStr).replace(/[^0-9.-]+/g, "")) || 0;

                const nazwaKarnetu = karnet.nazwa || 'Inny karnet';

                // Zestawienie wszystkich aktywnych karnetów
                ...
                // Sprawdzenie czy karnet jest na UMOWĘ
                if (isKarnetNaUmowe(karnet)) {
                  aktywneUmowy++;
                  przychodStalyUmowy += cenaNum;

                  if (!umowyMap[nazwaKarnetu]) {
                    umowyMap[nazwaKarnetu] = { ilosc: 0, sumaCen: 0 };
                  }
                  umowyMap[nazwaKarnetu].ilosc++;
                  umowyMap[nazwaKarnetu].sumaCen += cenaNum;
                }
              }
            });
        ...
        const umowyZestawienie = Object.entries(umowyMap).map(([nazwa, data]) => ({
          nazwa,
          ilosc: data.ilosc,
          lacznaWartosc: data.sumaCen
        })).sort((a, b) => b.ilosc - a.ilosc);
        ...
        // Generowanie osi czasu w odwróconej kolejności (od bieżącego miesiąca w lewo do 24 miesięcy wstecz w prawo)
        const chartSlots: ChartData[] = [];
        ...
        for (let i = 0; i < 24; i++) {
          ...
          chartSlots.push({ monthKey: key, label, value: 0 });
          monthLookup[key] = i;
        }
        ...
        if (transakcjeRaw) {
          transakcjeRaw.forEach((t: any) => {
            ...
            if (isKarnet && t.kwota) {
              ...
              const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
              if (monthLookup[key] !== undefined) {
                chartSlots[monthLookup[key]].value += Math.abs(Number(t.kwota));
              }
            }
          });
        }
    ...
    <div className="max-w-[1700px] mx-auto space-y-8 pb-24 relative font-sans antialiased">
      ...
      <div className="border-b border-sky-200 pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-1">Podsumowanie stanu klubu, umów, płatności i przychodów stałych na żywo</p>
        </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 space-y-4 animate-pulse">
          ...
          <div className="text-sm font-bold text-sky-900 uppercase tracking-wider">Synchronizacja z bazą danych...</div>
        </div>
      ) : (
        <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            {/* KOLUMNA 2 - WSZYSTKIE AKTYWNE KARNETY */}
            <div className="space-y-6">
            ...
            {/* KOLUMNA 3 - RAPORT UMÓW (STAŁY PRZYCHÓD MIESIĘCZNY) */}
            <div className="space-y-6">
              <div className="bg-white border border-sky-200 rounded-3xl p-6 space-y-4 shadow-sm">
                <h2 className="text-sm font-black uppercase tracking-wider text-sky-900 border-b border-sky-100 pb-3 flex items-center gap-2">
                ...
                <div className="bg-sky-50/70 border border-sky-200 rounded-2xl p-4 flex flex-col gap-3">
                  <div className="flex justify-between items-center border-b border-sky-200/60 pb-3">
                    <span className="text-[11px] font-bold text-slate-600 uppercase tracking-wider">Sprzedane umowy</span>
                    <div className="text-2xl font-black text-sky-900">{stats.aktywneUmowy}</div>
                  </div>
                  
                  <div className="bg-white border border-sky-200 px-4 py-3 rounded-xl flex justify-between items-center shadow-xs">
                    <span className="text-[10px] text-sky-800 font-black uppercase tracking-wider">Stały przychód z umów (MRR)</span>
                    <div className="text-sm font-black text-emerald-600">
                      {stats.przychodStalyUmowy.toFixed(2)} PLN/mc
                    </div>
                  </div>
                </div>

                {/* WYSZCZEGÓLNIENIE SPRZEDANYCH UMÓW */}
                <div className="space-y-2 pt-1">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Wyszczególnienie umów:</span>
                  <div className="max-h-[160px] overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
                    {stats.umowyZestawienie.length > 0 ? (
                      stats.umowyZestawienie.map((u, i) => (
                        <div key={i} className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                          <div className="flex items-center gap-1.5 truncate pr-2">
                            ...
                            <span className="font-bold text-slate-800 truncate">{u.nazwa}</span>
                            <span className="text-[10px] font-black text-slate-500 bg-white px-1.5 py-0.5 rounded border border-slate-200 shrink-0">x{u.ilosc}</span>
                          </div>
                          <span className="font-bold text-emerald-600 shrink-0">{u.lacznaWartosc.toFixed(2)} PLN/mc</span>
                        </div>
                      ))
                    ) : (
                      <div className="text-center py-3 text-slate-400 text-xs italic">Brak zarejestrowanych aktywnych umów.</div>
                    )}
                  </div>
                </div>
          ...
          {/* SEKCJA - WYKRES PRZYCHODÓW (24 MIESIĄCE: OD NAJNOWSZEGO PO LEWEJ DO NAJSTARSZEGO PO PRAWEJ) */}
