...
export default function ZasadyZapisowPage() {
  const [activeTab, setActiveTab] = useState<'rules' | 'auto_bookings'>('rules');
  ...
  // Stany dla automatycznych zapisów
  ...
  const loadData = async () => {
    try {
      ...
      // 4. Pobierz klientów do automatycznych zapisów
      ...
      // 5. Pobierz grafik zajęć i zajęcia jednorazowe
  ...
  // Obsługa przypisania użytkownika w zakładce Automatyczne Zapisy
  const handleAutoAssign = async (cls: any) => {
    ...
    try {
      ...
      if (insertErr) {
        console.warn("Informacja przy zapisie tabeli zapisy_zajec:", insertErr.message);
      }
      ...
      const newZapis = {
        ...
        zapisujacy: 'Automatyczne Zapisy (Panel Admina)'
      };
      ...
      await supabase.from('booking_logs').insert([
        {
          action_type: 'AUTO_BOOKING_ASSIGNED',
          status: 'SUCCESS',
          reason: `Automatycznie przypisano klubowicza ${clientName} na zajęcia: ${cls.title || cls.nazwa}`,
          rule_applied: 'AUTOMATYCZNE_ZAPISY',
        },
      ]);

      showToast(`Pomyślnie zapisano automatycznie: ${clientName} na ${cls.title || cls.nazwa}!`);
    } catch (err: any) {
  ...
  return (
    <div className="max-w-[1250px] mx-auto space-y-6 pb-16 font-sans antialiased text-slate-800">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[11px] font-black tracking-widest text-sky-300 uppercase">
              Silnik Reguł i Automatyzacji Klubowej
            </span>
          </div>
          <h1 className="text-2xl font-black uppercase tracking-wider text-white flex items-center gap-3">
            📋 ZASADY I AUTOMATYCZNE ZAPISY
          </h1>
          <p className="text-xs text-sky-200/80 font-medium">
            Zarządzaj nadrzędnymi regułami rezerwacji oraz przypisuj klubowiczów automatycznie do zajęć z grafiku.
          </p>
        </div>
      ...
      <div className="flex items-center gap-3 border-b border-slate-200 pb-3">
        ...
        <button
          onClick={() => setActiveTab('auto_bookings')}
          className={`px-5 py-2.5 rounded-2xl text-xs font-black tracking-wide transition-all cursor-pointer ${
            activeTab === 'auto_bookings'
              ? 'bg-sky-950 text-white shadow-md'
              : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'
          }`}
        >
      ...
      {/* ZAWARTOŚĆ ZAKŁADKI 1: ZASADY ZAPISÓW */}
      {activeTab === 'rules' && (
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-sm space-y-8 animate-fadeIn">
      ...
      {/* ZAWARTOŚĆ ZAKŁADKI 2: AUTOMATYCZNE ZAPISY (Styl spójny z zasadami) */}
      {activeTab === 'auto_bookings' && (
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6 animate-fadeIn">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-4 gap-2">
            <div className="space-y-1">
              <h2 className="text-xs font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
                ⚡ Automatyczne Przypisywanie do Grafiku Zajęć
              </h2>
              <p className="text-xs text-slate-400 font-medium">
                Wybierz zajęcia z grafiku i przypisz klubowicza. Zostanie on automatycznie zapisany bez potrzeby samodzielnego rezerwowania.
              </p>
            </div>
            <span className="text-[11px] font-black text-sky-900 bg-sky-50 px-3 py-1.5 rounded-xl border border-sky-200">
              Aktywnych pozycji: {grafikItems.length}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3.5">
            {grafikItems.length > 0 ? (
              grafikItems.map((cls, idx) => (
                <div
                  key={cls.id || idx}
                  className="flex flex-col md:flex-row md:items-center justify-between p-4 bg-slate-50/70 border border-slate-200/60 rounded-2xl hover:bg-slate-50 transition-colors gap-4"
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 rounded-lg text-[10px] font-black uppercase bg-sky-100 text-sky-900">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
            <div className="p-6 bg-sky-950 text-white flex items-center justify-between">
              <div>
                <h3 className="text-base font-black uppercase tracking-wider flex items-center gap-2">
                  ❓ Jak Działają Zasady i Automatyczne Zapisy?
                </h3>
                <p className="text-xs text-sky-200">Hierarchia reguł oraz automatyzacja rezerwacji.</p>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs text-slate-700 leading-relaxed">
              <div className="p-4 bg-sky-50 rounded-2xl border border-sky-100 space-y-2">
                <h4 className="font-black text-sky-950 uppercase">⚡ Automatyczne Zapisy</h4>
                <p>
                  Zakładka "Automatyczne Zapisy" pozwala administratorowi wybrać dowolne zajęcia z grafiku i bezpośrednio przypisać do nich klubowicza. System automatycznie dopisze rezerwację do bazy oraz zaktualizuje nadchodzące zapisy w profilu klienta.
                </p>
              </div>
