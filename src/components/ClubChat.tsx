...
export default function ClubChat() {
  ...
  // Główne zakładki widoku listy: wyłącznie Prywatne | Grupy
  const [activeTab, setActiveTab] = useState<"direct" | "groups">("direct");
  const [groupFilterTab, setGroupFilterTab] = useState<"my" | "public" | "closed">("my");

  // Rozwijanie sekcji czatów grupowych w zakładce Prywatne
  ...
  // Stan przypiętych czatów i grup
  ...
  useEffect(() => {
    ...
    if (savedActiveTab === "direct" || savedActiveTab === "groups") {
  ...
  const sendChatPushNotification = async (
    ...
    try {
      ...
      if (isTargetAdmin) {
        ...
        const { data: adminClients } = await (supabase.from("klienci") as any)
          .select('push_subscription, "E-mail"')
      ...
      console.error("Błąd podczas bezpiecznego wysyłania powiadomienia Push:", err);
    }
  };
  ...
  const sendGroupPushNotification = async (
    ...
    try {
      ...
      if (groupData) {
        ...
        recipientIds = members.filter((id) => id && !muted.includes(id) && !banned.includes(id));
      }
  ...
  useEffect(() => {
    const initUser = async () => {
      ...
      const { data: klienciData } = await (supabase.from("klienci") as any)
        .select('*')
      ...
      if (klienciData) {
        ...
        const myProfiles = enriched.filter((c: any) => c.email === userEmail);
        const primaryProfile = myProfiles[0];
        ...
        if (adminLogged) {
          ...
          setCurrentUserName(primaryProfile.name);
          setCurrentUserAvatar(primaryProfile.avatar);
          updateLastSeen(primaryProfile.id);
        }
      }
    };
  ...
  // POBIERANIE PEŁNEJ HISTORII DEDYKOWANEJ DLA OTWARTEJ ROZMOWY 1:1
  const fetchDirectMessages = async (targetUser: any) => {
    ...
    try {
      ...
      if (isSys) {
        query = query
          .or(`nadawca_id.eq.${SYSTEM_ID},nadawca_id.is.null,nadawca_rola.eq.system`)
          .in("odbiorca_id", myIds);
      } else {
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      if (!error && data) {
        ...
        console.error("Błąd zapytania czat_wiadomosci:", error);
      }
    } catch (err) {
    ...
    channel
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === "INSERT") {
            ...
            if (selectedGroupRef.current && String(updatedRow.grupa_id) === String(selectedGroupRef.current.id)) {
              setGroupMessages((prev) => prev.map((m) => (m.id === updatedRow.id ? updatedRow : m)));
            }
      ...
      .on("broadcast", { event: "typing" }, ({ payload }) => {
        ...
        const isCurrentGroupOpen =
          ...
          String(selectedGroupRef.current.id) === String(groupId) &&
          !myActualIds.includes(String(senderId));

        if (isCurrentDirectOpen || isCurrentGroupOpen) {
          const senderKey = String(senderId);

          if (typingTimeoutRef.current[senderKey]) {
            clearTimeout(typingTimeoutRef.current[senderKey]);
          }

          setTypingUsers((prev) => ({
            ...prev,
            [senderKey]: { name: senderName, timestamp: Date.now() },
          }));

          typingTimeoutRef.current[senderKey] = setTimeout(() => {
            setTypingUsers((prev) => {
              const copy = { ...prev };
              delete copy[senderKey];
              return copy;
  ...
  }, [currentUserId, secondaryUserId, currentUserEmail, isOpen, isAdmin]);
  ...
  const handleTypingBroadcast = () => {
    ...
    if (!realtimeChannelRef.current || (!selectedUser && !selectedGroup)) return;
    ...
    realtimeChannelRef.current.send({
      ...
      payload: {
        ...
        targetUserId: selectedUser ? selectedUser.id : null,
  ...
  }, [isOpen, selectedUser, currentUserId, secondaryUserId, currentUserEmail, isAdmin]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (selectedGroup) {
      ...
      let { data, error } = await supabase.from("czat_wiadomosci").insert([payload]).select();
      ...
      if (error && (error.message?.includes("reply_to") || error.code === "PGRST204" || error.code === "42703")) {
        ...
        const retry = await supabase.from("czat_wiadomosci").insert([payload]).select();
        data = retry.data;
        error = retry.error;
      }
      ...
      if (error) {
        ...
        if (data && data.length > 0) {
          ...
          setGroupMessages((prev) => (prev.some((m) => m.id === sentRow.id) ? prev : [...prev, sentRow]));
  ...
  // PRECYZYJNY FILTR WIADOMOŚCI 1:1 DLA AKTYWNEGO OKNA CZATU
  const activeChatMessages = messages.filter((m: any) => {
    ...
    // Relacja klubowicz <-> Administrator
    if (isSelectedUserAdmin) {
      const isSenderAdmin = allAdminIds.includes(sId) || m.is_system || m.nadawca_rola === "system";
      const isReceiverAdmin = allAdminIds.includes(rId);

      if (isAdmin) {
        return (isSenderMe && isReceiverAdmin) || (isSenderAdmin && isReceiverMe);
      } else {
        return (isSenderAdmin && isReceiverMe) || (isSenderMe && isReceiverAdmin);
      }
    }

    if (Number(selectedUser.id) === SYSTEM_ID) {
      const isSysMsg = m.nadawca_id === null || Number(m.nadawca_id) === SYSTEM_ID || m.is_system || m.nadawca_rola === "system";
  ...
  messages.forEach((m: any) => {
    if (!m.grupa_id) {
      ...
      if (effectiveIds.includes(sId)) {
        let targetId = rId;
        if (!isAdmin && allAdminIds.includes(targetId)) {
          const maciejAccount = klienci.find((k: any) => ADMIN_EMAILS.includes(k.email));
          if (maciejAccount) targetId = String(maciejAccount.id);
        }
        chattedUserIds.add(targetId);
      } else if (effectiveIds.includes(rId)) {
        let senderId = sId;
        if (!isAdmin && allAdminIds.includes(senderId)) {
          const maciejAccount = klienci.find((k: any) => ADMIN_EMAILS.includes(k.email));
          if (maciejAccount) senderId = String(maciejAccount.id);
        }
        chattedUserIds.add(senderId);
      }
    }
  });
  ...
  const displayedUsers = klienci
    ...
    .sort((a, b) => {
      const isSysA = Number(a.id) === SYSTEM_ID;
      const isTargetAdminA = allAdminIds.includes(String(a.id)) || ADMIN_EMAILS.includes(a.email);
      const unreadA = messages.filter((m: any) => {
        if (m.grupa_id || m.przeczytana) return false;
        if (!effectiveIds.includes(String(m.odbiorca_id))) return false;
        if (isSysA) return m.nadawca_id === null || Number(m.nadawca_id) === SYSTEM_ID || m.nadawca_rola === "system";
        if (isTargetAdminA) return allAdminIds.includes(String(m.nadawca_id)) || m.is_system || m.nadawca_rola === "system";
        return String(m.nadawca_id) === String(a.id);
      }).length;

      const isSysB = Number(b.id) === SYSTEM_ID;
      const isTargetAdminB = allAdminIds.includes(String(b.id)) || ADMIN_EMAILS.includes(b.email);
      const unreadB = messages.filter((m: any) => {
        if (m.grupa_id || m.przeczytana) return false;
        if (!effectiveIds.includes(String(m.odbiorca_id))) return false;
        if (isSysB) return m.nadawca_id === null || Number(m.nadawca_id) === SYSTEM_ID || m.nadawca_rola === "system";
        if (isTargetAdminB) return allAdminIds.includes(String(m.nadawca_id)) || m.is_system || m.nadawca_rola === "system";
  ...
  const allMyGroups = groups.filter((g: any) => {
    if (g.typ === "trening" || g.nazwa?.startsWith("Trening:")) return false;
  ...
  const unreadDirect1on1Count = messages.filter((m: any) => {
    ...
    const isForMe = effectiveIds.includes(rId);
    if (!isForMe || effectiveIds.includes(sId)) return false;
  ...
  const MessageItem = ({ msg, isMe }: { msg: any; isMe: boolean }) => {
    const isSystemSender = Number(msg.nadawca_id) === SYSTEM_ID || msg.nadawca_id === null || msg.is_system || msg.nadawca_rola === "system";
    const rawText = msg.tresc || "";

    const isTrainer5MinReminder = rawText.includes("[REMINDER_5MIN_");
    const isTrainerEndOfDayReminder = rawText.includes("[REMINDER_END_OF_DAY_");

    const cleanDisplayContent = rawText
      .replace(/\[REMINDER_5MIN_[^\]]+\]/g, "")
      .replace(/\[REMINDER_END_OF_DAY_[^\]]+\]/g, "")
      .trim();

    const isRedukcjaAlert = isSystemSender && (rawText.includes("Wyzwanie") || rawText.includes("redukcji") || rawText.includes("Redukcji") || rawText.includes("KOMUNIKAT SYSTEMOWY"));
    const isBirthdayNotification = isSystemSender && (rawText.includes("🎂") || rawText.includes("urodzin"));
    const isBadgeNotification = isSystemSender && (rawText.includes("🎖️") || rawText.includes("odznakę klubową"));
    const isChallengeNotification = rawText.includes("⚔️") || rawText.includes("Rzuciłem Ci wyzwanie");
    const isKnowledgeBaseNotification = isSystemSender && (rawText.includes("Bazy Wiedzy") || rawText.includes("Baza Wiedzy") || rawText.includes("suplemencie"));

    const isSpecial =
      isTrainer5MinReminder ||
      isTrainerEndOfDayReminder ||
      isSystemSender ||
      isBirthdayNotification ||
      isBadgeNotification ||
      isChallengeNotification ||
      isKnowledgeBaseNotification ||
      isRedukcjaAlert;
        ...
        <div className="w-full bg-gradient-to-br from-amber-500/20 via-slate-900 to-sky-950 border-2 border-amber-400 rounded-3xl p-4 shadow-xl text-white space-y-3">
          <div className="flex items-center justify-between border-b border-amber-400/40 pb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="font-black text-[11px] uppercase tracking-wider text-amber-300">
                Przypomnienie dla Trenera: Za 5 min trening
              </span>
            </div>
            <span className="text-[9px] bg-amber-400 text-slate-950 font-black px-2 py-0.5 rounded-full uppercase shadow-xs">
              Obecność
            </span>
          </div>
          ...
          <div className="pt-1 flex justify-end">
            <button
              ...
              className="bg-amber-400 hover:bg-amber-500 text-slate-950 font-black text-xs px-3.5 py-1.5 rounded-xl transition-all shadow-sm cursor-pointer"
            >
              Przejdź do grafiku i sprawdź obecność ➔
            </button>
          </div>
        </div>
      );
        ...
        <div className="w-full bg-gradient-to-br from-emerald-500/20 via-slate-900 to-slate-950 border-2 border-emerald-400 rounded-3xl p-4 shadow-xl text-white space-y-3">
          <div className="flex items-center justify-between border-b border-emerald-400/40 pb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="font-black text-[11px] uppercase tracking-wider text-emerald-300">
                Podsumowanie Dnia Trenera
              </span>
            </div>
            <span className="text-[9px] bg-emerald-400 text-slate-950 font-black px-2 py-0.5 rounded-full uppercase shadow-xs">
              Weryfikacja
            </span>
          </div>
          <p className={`${getChatTextClass("message")} leading-relaxed font-semibold text-slate-100 break-words`}>
          ...
          <div className="pt-1 flex justify-end">
            <button
              type="button"
              onClick={() => handleCloseChat()}
              className="bg-emerald-400 hover:bg-emerald-500 text-slate-950 font-black text-xs px-3.5 py-1.5 rounded-xl transition-all shadow-sm cursor-pointer"
            >
              Otwórz grafik i zweryfikuj listy ➔
            </button>
          </div>
        </div>
      );
  ...
  const renderDirectUserItem = (user: any, isPinnedItem: boolean = false) => {
    ...
    const isTargetAdmin =
      allAdminIds.includes(String(user.id)) ||
      ADMIN_EMAILS.includes(user.email) ||
      user.name?.toLowerCase().includes("maciej kłaput");

    const userUnread = messages.filter((m: any) => {
      const isForMe = effectiveIds.includes(String(m.odbiorca_id));

      if (!isForMe || m.grupa_id || m.przeczytana) return false;
      if (isSys) {
        return m.nadawca_id === null || Number(m.nadawca_id) === SYSTEM_ID || m.nadawca_rola === "system";
      }
      if (isTargetAdmin) {
        return allAdminIds.includes(String(m.nadawca_id)) || m.is_system || m.nadawca_rola === "system";
      }
          ...
          onClick={() => {
            setMessages((prev) =>
              prev.map((m) => {
                ...
                const isTargetReceiver = effectiveIds.includes(rId);
    ...
    >
      {isOpen && (
        ...
        >
          ...
          <div className="bg-slate-900 text-white px-3 py-2.5 flex items-center justify-between shadow-sm select-none relative shrink-0">
                    ...
                    <>
                      ...
                      <div className="overflow-hidden min-w-0 flex-1">
                        ...
                        <div className="text-[9px] text-amber-400 font-medium truncate">
                          ...
                          {selectedGroup.typ === "publiczna" ? "Publiczna" : "Zamknięta"} • {getGroupMembersCount(selectedGroup)} os.
                        </div>
                      </div>
                    </>
                  ) : (
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button
                ...
                className="text-slate-400 hover:text-white w-7 h-7 rounded-full bg-slate-800 flex items-center justify-center font-bold text-xs cursor-pointer transition-colors shrink-0"
              >
                ✕
              </button>
            </div>
          ...
          {/* WIDOK GŁÓWNY (LISTA ROZMÓW / GRUP - UKŁAD DWÓCH ZAKŁADEK) */}
          {!selectedUser && !selectedGroup ? (
              ...
              <div className="flex items-center justify-between gap-1.5 border-b border-slate-200 pb-2 shrink-0">
                <div className="flex-1 grid grid-cols-2 gap-1 bg-slate-200/90 p-1 rounded-xl">
                  <button
                    ...
                    className={`py-1.5 rounded-lg text-xs font-bold transition-all relative flex items-center justify-center gap-1 cursor-pointer ${
              ...
              {activeTab === "direct" && (
                <>
                  ...
                  <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 min-h-0">
                    ...
                    {/* POZOSTAŁE ROZMOWY 1-NA-1 (Z NIEPRZECZYTANYMI NA POCZĄTKU) */}
            ...
            /* WIDOK LISTY UCZESTNIKÓW W GRUPIE */
            <div className="flex-1 flex flex-col overflow-hidden bg-slate-100 p-3 space-y-3 min-h-0">
              ...
              {isAdmin && groupPendingRequestsList.length > 0 && (
                <div className="bg-amber-50 border border-amber-300 rounded-2xl p-2.5 space-y-2 shadow-sm shrink-0">
                  ...
                  <div className="flex items-center gap-2 text-slate-500 text-xs italic px-1 py-1 animate-in fade-in">
                    <div className="flex items-center gap-1.5 bg-white border border-slate-200 px-3 py-1.5 rounded-2xl shadow-xs">
                      <span className="text-[10px] font-bold text-slate-600">
                        {Object.values(typingUsers).map((u) => u.name.split(" ")[0]).join(", ")} pisze wiadomość
                      </span>
                  ...
                  <div className="py-12 text-center text-slate-400 text-xs space-y-1">
                    ...
                    <p className="text-[10px]">
                      {selectedGroup
                        ...
                        : Number(selectedUser?.id) === SYSTEM_ID
                        ? "Tutaj pojawiać się będą oficjalne powiadomienia o odznakach, suplementach, urodzinach, wydarzeniach i obecnościach."
          ...
          {showCategoryManagerModal && (
            <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-3 animate-in fade-in">
              <div className="bg-white rounded-3xl p-4 w-full max-w-sm max-h-[88vh] flex flex-col shadow-2xl border border-slate-200 space-y-3">
                <div className="flex items-center justify-between border-b pb-2 shrink-0">
                  ...
                  <button type="button" onClick={() => setShowCategoryManagerModal(false)} className="text-slate-400 hover:text-slate-600 font-bold cursor-pointer">
                    ✕
                  </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-1.5 bg-slate-50 p-2 rounded-xl border border-slate-200 min-h-0">
            ...
            <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-3 animate-in fade-in">
              <div className="bg-white rounded-3xl p-4 w-full max-w-sm max-h-[88vh] flex flex-col shadow-2xl border border-slate-200 space-y-3">
                <div className="flex items-center justify-between border-b pb-2 shrink-0">
                  ...
                  <button type="button" onClick={() => setShowInviteModal(false)} className="text-slate-400 hover:text-slate-600 font-bold cursor-pointer">
                    ✕
                  </button>
                </div>

                <div className="space-y-2 flex-1 flex flex-col overflow-hidden min-h-0">
                  <input
                    type="text"
                    placeholder="Filtruj klubowiczów..."
                    value={inviteSearchQuery}
                    onChange={(e) => setInviteSearchQuery(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-800 focus:outline-none focus:border-amber-500 shrink-0"
                  />

                  <div className="flex-1 overflow-y-auto space-y-1 bg-slate-50 p-2 rounded-xl border border-slate-200 min-h-0">
                    {klienci
                      .filter((k) => {
          ...
          {showEditGroupModal && (
            <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-3 animate-in fade-in">
              <div className="bg-white rounded-3xl p-4 w-full max-w-sm max-h-[88vh] flex flex-col shadow-2xl border border-slate-200 space-y-3 overflow-hidden">
                <div className="flex items-center justify-between border-b pb-2 shrink-0">
                  <div className="font-black text-xs uppercase tracking-wider text-slate-900 flex items-center gap-2">
                    <span>✏️</span> Edytuj Grupę
                  </div>
                  <button type="button" onClick={() => setShowEditGroupModal(false)} className="text-slate-400 hover:text-slate-600 font-bold cursor-pointer">
                    ✕
                  </button>
                </div>

                <form onSubmit={handleUpdateGroup} className="space-y-3 flex-1 overflow-y-auto pr-1 min-h-0">
                  <div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-1 mt-2 text-base flex-wrap">
                      {["🏋️‍♂️", "🚴", "🏕️", "🥇", "🔥", "⚽", "🥊", "💪"].map((emo) => (
                        <button
                          key={emo}
                          type="button"
            ...
            <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-3 animate-in fade-in">
              <div className="bg-white rounded-3xl p-4 w-full max-w-sm max-h-[88vh] flex flex-col shadow-2xl border border-slate-200 space-y-3 overflow-hidden">
                <div className="flex items-center justify-between border-b pb-2 shrink-0">
                  ...
                  <button type="button" onClick={() => setShowBroadcastModal(false)} className="text-slate-400 hover:text-slate-600 font-bold cursor-pointer">
                    ✕
                  </button>
                </div>

                <form onSubmit={handleSendBroadcast} className="space-y-3 flex-1 overflow-y-auto pr-1 min-h-0">
                  <textarea
                    rows={4}
            ...
            <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-3 animate-in fade-in">
              <div className="bg-white rounded-3xl p-4 w-full max-w-sm max-h-[88vh] flex flex-col shadow-2xl border border-slate-200 space-y-3 overflow-hidden">
                <div className="flex items-center justify-between border-b pb-2 shrink-0">
                  ...
                  <button type="button" onClick={() => setShowCreateGroupModal(false)} className="text-slate-400 hover:text-slate-600 font-bold cursor-pointer">
                    ✕
                  </button>
                </div>

                <form onSubmit={handleCreateGroup} className="space-y-3 flex-1 overflow-y-auto pr-1 min-h-0">
                  <input
                    type="text"
                  ...
                  <div>
                    ...
                    <div className="flex gap-1 mt-1.5 text-base flex-wrap">
                      {["🏋️‍♂️", "🚴", "🏕️", "🥇", "🔥", "⚽", "🥊", "💪"].map((emo) => (
                        <button
                          key={emo}
                          type="button"
